...
# ─── HITTER REPORT FUNCTION (unchanged) ───────────────────────────────────────
...
if uploaded_file:
    ...
    if 'Date' not in df_all.columns:
        ...
        selected_date = col2.selectbox("Game Date", dates, key="game_date")
        df_d = df[df['Date']==selected_date]

        if report == "Pitcher Report":
            names  = sorted(df_d['Pitcher'].unique())
            player = col3.selectbox("Pitcher", names, key="pitcher_name")
            st.subheader(f"{player} — {selected_date}")
            fig_sum = combined_pitcher_report(df_d, player, LOGO_PATH, coverage=0.8)
            ...
            names  = sorted(df_d['Batter'].unique())
            player = col3.selectbox("Batter", names, key="batter_name")
            st.subheader(f"{player} — {selected_date}")
            fig = create_hitter_report(df_d, player, ncols=3)
            st.pyplot(fig)
